...
### Hosting on the filesystem

If you need to share the resulting site outside of a web server (on a local network share for documentation, on a thumb drive, etc.), you will need relative URLs that link to the exact file paths.

This can be achieved with this config:
...
Note that this plugin only rewrites URLs that are generated by Kirby or by the `url()` helper function.
